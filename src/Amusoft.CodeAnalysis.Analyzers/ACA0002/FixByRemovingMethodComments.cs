...
{
	[ExportCodeFixProvider(LanguageNames.CSharp, Name = "Amusoft.CodeAnalysis.Analyzers.ACA0002-FixByRemovingMethodComments"), Shared]
	public class FixByRemovingMethodComments : SingleDiagnosticDocumentCodeFixProviderBase
	{
		/// <inheritdoc />
		protected override string DiagnosticId { get; } = DiagnosticIds.ACA0002.DiagnosticOnMethod;

		/// <inheritdoc />
		protected override string GetEquivalenceKey(SyntaxNode rootNode)
		{
			return $"{DiagnosticId}-FixByRemovingMethodComments";
		}

		/// <inheritdoc />
		protected override string GetTitle(SyntaxNode rootNode)
		{
			return Resources.CommentAnalyzer_MethodRuleCodeFixMessage;
		}
			...
			CancellationToken cancellationToken)
		{
			...
			{
				return CommentRemovalUtility.RewriteDocument(document, syntaxRoot, methodDeclarationSyntax);
			}
